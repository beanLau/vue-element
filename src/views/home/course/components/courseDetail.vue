    ...
    >
      <el-row>
        <el-col :span="12">
          ...
          <el-form-item label="课程封面" prop="logoImg">
            <el-upload
              action="/base/upload/"
            ...
            <span class="form-tip">比例要求 第一张16：9；第二张1：1</span>
          </el-form-item>
        ...
        <el-col :span="12">
          ...
          <el-form-item label="购买基数" prop="base">
            <el-input
              v-model="inputForm.base"
              placeholder="购买基数"
              class="custom-width"
              clearable
            ></el-input>
      ...
      <div class="el-row">
        <el-col :span="20">
          <el-form-item label="内容" prop="detailWeb">
            <mavon-editor
              v-model="inputForm.detailWeb"
              @imgAdd="imgAdd"
              ref="md"
              :toolbars="toolbars"
            />
...
import { getCourseById, addCourse, modifyCourse } from "@/api/course";
import { positiveIntegerValidater2, priceValidater } from "@/utils/validate";
export default {
  data() {
    let _this = this;
    ...
    return {
      ...
      rules: {
        ...
        sort: {
          ...
          message: "请输入排序值",
          trigger: "blur",
          validator: positiveIntegerValidater2
        },
        price: {
          required: true,
          validator: priceValidater
        },
        base: {
          required: true,
          message: "请输入排序值",
  ...
  methods: {
    setBreadcrumb() {
      if (this.$route.query.id) {
        window.Breadcrumb.setBreadcrumbLastName("修改课程");
      }
    },
    getSelectData() {
    ...
    getPageData() {
      getCourseById({
        ...
        .then(res => {
          ...
          resData.sysClassId = resData.sysClassId + "";
          this.inputForm = {
            name: resData.name,
            sysClassId: resData.sysClassId,
            teacherIds: resData.teacherIds.split(","),
            sort: resData.sort,
            detailWeb: resData.detailWeb,
          ...
          if (that.$route.query.id) {
            ...
            addCourse(options)
              .then(() => {
                ...
                that.loading = false;
                that.publish("course"); //发送通知，更新页面数据
                that.$router.back();
              })
              .catch(() => {
                that.loading = false;
